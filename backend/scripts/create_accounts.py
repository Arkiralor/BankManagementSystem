...
class FakeAccount:
    ...
    TYPES_OF_ACCOUNTS = [AccountChoice.savings, AccountChoice.current, AccountChoice.loan, AccountChoice.credit]

    @classmethod
    def get_account_holders(cls, count=1):
        if count <= 0:
            raise ValueError("There cannot be 0 account holders for an account.")
    ...
    def create_account(cls):
        ...
        resp = AccountHelpers.create(customers=holder, balance=float(balance), account_type=account_type)
        if resp.error:
